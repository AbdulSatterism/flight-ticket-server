...
const getFlightsBySearching = async (query:{origin?:string; destination?:string; date?:string}) => {
...
// get flight by id 
  const getFlightById = async (id: string) => {
    ...
    if (!flightExist) {
      throw new AppError(httpStatus.BAD_REQUEST,'This flight is not available in database');
    }
  ...
  const deleteFlightByAdmin = async (id: string) => {
    ...
    if (!flightExist) {
      throw new AppError(
        httpStatus.BAD_REQUEST,'This Flight is not available in database' );
    }
